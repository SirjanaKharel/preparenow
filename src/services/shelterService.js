...
const getCachedShelters = async (lat, lon) => {
  try {
    ...
    console.warn('Firestore cache unavailable, fetching live:', err.message);
    return null;
  }
};
...
const cacheShelters = async (lat, lon, shelters) => {
  try {
    ...
    console.warn(' Firestore cache write skipped:', err.message);
  }
};
...
const fetchFromOverpass = async (lat, lon) => {
  ...
  while (attempts < MAX_RETRIES && endpointsTried < OVERPASS_ENDPOINTS.length) {
    ...
    console.log(
      `🌐 Overpass attempt ${attempts + 1}/${MAX_RETRIES} via ${endpoint} ` +
    ...
    try {
      ...
      if (response.status === 403) {
        // Forbidden: rotate endpoint, do not retry this endpoint
        console.warn(`Overpass 403 Forbidden — rotating endpoint, not retrying this endpoint.`);
        endpointIndex++;
        endpointsTried++;
        lastError = new Error(`Overpass API error: 403`);
        continue;
      }

      if (response.status === 429 || response.status >= 500) {
        const backoff = jitter(1000 * 2 ** attempts);
        console.warn(`Overpass ${response.status} — retrying in ${Math.round(backoff)}ms`);
        endpointIndex++;
        endpointsTried++;
      ...
      if (!contentType.includes('application/json') && !contentType.includes('text/json')) {
        const backoff = jitter(1000 * 2 ** attempts);
        console.warn(`Overpass returned non-JSON (${contentType}) — rotating mirror, retrying in ${Math.round(backoff)}ms`);
        endpointIndex++;
        endpointsTried++;
      ...
      if (attempts < MAX_RETRIES - 1) {
        const backoff = jitter(1000 * 2 ** attempts);
        console.warn(`Overpass fetch error — retrying in ${Math.round(backoff)}ms:`, err.message);
        endpointIndex++;
        endpointsTried++;
        await sleep(backoff);
      }
...
export const shelterService = {
  ...
  clearCache: async () => {
    try {
      ...
      await batch.commit();
      console.log(` Cleared ${snap.size} cached shelter documents`);
    } catch (err) {

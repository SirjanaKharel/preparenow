...
let isInitialAppLoad = true; // Prevents exit notifications on app start

let DISASTER_ZONES = []; // Disaster zones loaded from Firebase
let zonesListener  = null;

let DEVELOPER_MODE = false; // Enable test location
let TEST_LOCATION  = null;

let lastCheckedLocation = null; // Used to avoid duplicate checks

let locationChangeListeners = []; // Listeners for location changes
...
let eventChangeListeners = []; // Listeners for event changes
...
// Configure notification handler for disaster alerts
...
// Listen for real-time disaster zones from Firebase
export const subscribeToDisasterZones = (callback) => {
  try {
    ...
    zonesListener = onSnapshot(q, (snapshot) => {
      ...
      // Important: Loaded disaster zones (keep for debugging)
...
// Load disaster zones once from Firebase
const loadDisasterZonesOnce = async () => {
  try {
    ...
    if (DISASTER_ZONES.length === 0) {
      console.warn(' No disaster zones found. Run the GDACS sync script.');
    }
  } catch (error) {
...
// Enable or disable developer mode (for testing location)
...
// Check if user has exited any zones (dev mode)
const manualCheckExits = async (currentCoords) => {
  try {
    ...
    for (const zoneId of activeZoneIds) {
      ...
      if (dist > zone.radius) {
        // Important: Dev mode exit detected (keep for debugging)
...
// Check if a notification should be sent (cooldown logic)
...
// Geofencing background task for zone entry/exit
...
// Handle entering a disaster zone
...
// Handle exiting a disaster zone
...
// Send a push notification for disaster alert
...
// Get notification message for alert
...
export const locationService = {
  ...
  // Start geofencing monitoring for disaster zones
  ...
  // Get all stored disaster events
  ...
  // Get only critical disaster events
  ...
  // Clear all stored disaster events
  ...
  // Check if user is inside a zone
  ...
  // Get all zones the user is currently inside
  ...
  // Get all live alerts from Firestore (last 24h)
  getLiveAlerts: async () => {
    try {
  ...
  // Get all critical alerts from Firestore (last 24h)
  ...
  // Manually check all zones for entry/exit (used in dev mode)
  ...
  // Get all loaded zones and their count
  getAllZones:  () => DISASTER_ZONES,
  getZoneCount: () => DISASTER_ZONES.length,
};

// Silently record zones on startup 

...
export default function PlanScreen({ navigation }) {
  ...
  // Load plan and shelters on mount
  useEffect(() => {
    if (user?.uid) {
      loadData(user.uid);
    }
  ...
  // Update map and shelters when location changes
  useEffect(() => {
  ...
  // Load shelters for given coordinates
